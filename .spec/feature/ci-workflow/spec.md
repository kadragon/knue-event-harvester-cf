...
## Behaviour (GWT)
- GIVEN a pull request is opened to `main` WHEN the workflow runs THEN it installs deps with the pinned Node version and fails if lint/test/typecheck fail.
- GIVEN the project uses Node 22 locally WHEN CI runs THEN it uses Node 22 to avoid version drift.

## Acceptance Tests
1. GitHub Actions workflow `CI` installs dependencies and runs `npm run lint`, `npm run test`, and `npm run typecheck` on PRs to `main`.
